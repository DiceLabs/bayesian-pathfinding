...
class BayesianRepulsionUpdater:
    ...
    def perform_update(self):
        ...
        prompt_lines = [
            ...
            "Work station is very very empty today, go to destination",
        ...
        try:
            ...
            print("[PARSE ERROR] Failed to parse GPT response:")
            print(raw_content if 'raw_content' in locals() else response.text)
            return
        ...
        for obs, prior in self._priors.items():
            ...
            if name not in likelihood_map:
                print(f"[WARN] No likelihood for '{name}', skipping.")
                continue
            ...
            print(f"[Bayes] {name}: prior={prior:.2f}, like={likelihood:.2f} and post={post:.2f}")
