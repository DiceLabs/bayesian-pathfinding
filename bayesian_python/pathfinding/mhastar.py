...
class MultiHeuristicAStar:
    ...
    def plan_path(self, start_position, goal_position, bayes_manager=None, repulsive_field=None, obstacles=None, visualize_fn=None):
        if start_position is None or goal_position is None:
            print("Error: Please assign start and goal positions.")
            return

        if bayes_manager is not None:
            bayes_manager.perform_bayesian_update()
        else:
            print("Warning: No RepulsiveBayesManager found; skipping Bayesian step.")
        ...
        print(f"[Grid] Total nodes: {total_nodes}")
        ...
        print(f"[Grid] Total nodes: {len(self.grid_gen.get_all_nodes())}")
        print(f"Start grid[{self.start_node.grid_x},{self.start_node.grid_y}] walkable? {self.start_node.walkable}")
        print(f"Goal grid[{self.goal_node.grid_x},{self.goal_node.grid_y}] walkable? {self.goal_node.walkable}")
        ...
        print(f" Path length = {total_world_length:.2f} units, "
