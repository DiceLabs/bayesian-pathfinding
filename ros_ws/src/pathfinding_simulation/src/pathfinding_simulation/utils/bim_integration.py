...
class BIMIntegration:
    ...
    def load_bim_obstacles(self):
        ...
        rospy.loginfo(f"Loading BIM data from {self.bim_json_path}")
        ...
        for obj_name, obj_data in bim_data.items():
            ...
            if any(skip_name in name_lower for skip_name in ['agent', 'capsule']):
                rospy.loginfo(f"  Skipping navigation/equipment object: {obj_name}")
                skipped_count += 1
                continue
        ...
        rospy.loginfo(f"Loaded {len(self.bim_obstacles)} BIM obstacles for Bayesian analysis")
        rospy.loginfo(f"Skipped {skipped_count} objects (navigation waypoints, equipment, or invalid)")
        return self.bim_obstacles
        
    def _get_default_danger(self, obj_name):

        name_lower = obj_name.lower()
        #TODO: This is the part I am not sure about
        if 'wall' in name_lower:
            return 0.5  
        elif 'workstation' in name_lower:
            return 0.5 
    ...
    def create_repulsive_obstacles(self):
        ...
        rospy.loginfo(f"Created {len(self.repulsive_obstacles)} repulsive obstacles")
    ...
    def update_repulsive_gains(self):
        ...
        rospy.loginfo("Updated repulsive obstacle gains from Bayesian results")
    ...
    def print_obstacle_summary(self):
        ...
        for category, names in categories.items():
            rospy.loginfo(f"{category}: {len(names)} objects")
            for name in names[:3]:  # Show first 3 examples
                rospy.loginfo(f"  - {name}")
            if len(names) > 3:
                rospy.loginfo(f"  ... and {len(names) - 3} more")
