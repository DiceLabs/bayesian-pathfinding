...
fbx_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', 'models', 'bim', 'dicelab_bim.fbx')
...
output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'utils', 'extracted_objects_new.json')
with open(output_file, 'w') as f:
    json.dump(object_data, f, indent=4)

rospy.loginfo(f"Extracted {len(object_data)} objects with enriched metadata.")
