...
class DynamicRepulsionManager:
    ...
    def _update_grid_cache(self):
        ...
        rospy.loginfo(f"[DynamicRepulsionManager] Grid size: {self.grid_size_x} x {self.grid_size_y}")
    ...
    def add_dynamic_obstacle(self, position: Tuple[float, float, float], name: str = "DynamicObstacle"):
        ...
        rospy.loginfo(f"[DynamicRepulsionManager] Added dynamic obstacle '{name}' at {position}")
        return obstacle
    ...
    def remove_dynamic_obstacle(self, obstacle: DynamicObstacle):
        ...
        if obstacle in self.dynamic_obstacles:
            ...
            rospy.loginfo(f"[DynamicRepulsionManager] Removed dynamic obstacle '{obstacle.name}'")
    
    def clear_dynamic_obstacles(self):
      
        count = len(self.dynamic_obstacles)
        self.dynamic_obstacles.clear()
        rospy.loginfo(f"[DynamicRepulsionManager] Cleared {count} dynamic obstacles")
    ...
    def apply_dynamic_repulsion(self):
        ...
        if self.grid is None:
            rospy.loginfo("[DynamicRepulsionManager] Grid is None, cannot apply repulsion")
            return
    ...
    def save_dynamic_potential(self, 
        ...
        if self.grid is None:
            rospy.loginfo("[DynamicRepulsionManager] No grid found - cannot export.")
            return
        ...
        if save_numpy:
            ...
            rospy.loginfo(f"[DynamicRepulsionManager] Saved NumPy array → {numpy_path}")
        ...
        if save_csv:
            ...
            rospy.loginfo(f"[DynamicRepulsionManager] Saved CSV: {csv_path}")
        ...
        rospy.loginfo(f"[DynamicRepulsionManager] Saved metadata → {metadata_path}")
    ...
    def set_influence_radius(self, radius: float):
        ...
        rospy.loginfo(f"[DynamicRepulsionManager] Set influence radius to {self.dynamic_influence_radius}")
    
    def set_dynamic_k(self, k_value: float):
       
        self.dynamic_k = max(0.0, k_value)
        rospy.loginfo(f"[DynamicRepulsionManager] Set dynamic K to {self.dynamic_k}")
    
    def print_status(self):
       
        active_count = self.get_obstacle_count()
        rospy.loginfo(f"[DynamicRepulsionManager] Status:")
        rospy.loginfo(f"  Grid size: {self.grid_size_x} x {self.grid_size_y}")
        rospy.loginfo(f"  Active obstacles: {active_count}")
        rospy.loginfo(f"  Influence radius: {self.dynamic_influence_radius}")
        rospy.loginfo(f"  Dynamic K: {self.dynamic_k}")
        
        if active_count > 0:
            field = self.get_dynamic_potential_field()
            max_potential = np.max(field)
            min_potential = np.min(field)
            rospy.loginfo(f"  Potential range: {min_potential:.4f} to {max_potential:.4f}")